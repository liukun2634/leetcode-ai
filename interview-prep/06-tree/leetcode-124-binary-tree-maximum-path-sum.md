...
### 自我提问
1. 为什么返回时只能选一支？→ 父亲接上后还要继续往上走，X 形路径不合法。
2. 负贡献为什么裁掉？→ 加进来只会更小；裁掉等于"这一支不走"。
3. 全负树（如 `[-3]`）怎么办？→ `maxSum` 初始化 `MIN_VALUE`，会被 `-3` 更新成 `-3`。
4. 怎么 **返回具体的路径**？→ 在更新 `maxSum` 时同步记录 "(leftPath, node, rightPath)"。
