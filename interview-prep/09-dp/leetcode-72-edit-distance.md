...
### 自我提问
1. 为什么三种操作分别对应 `dp[i-1][j-1] / dp[i-1][j] / dp[i][j-1]`？
   - 替换：`word1[i-1]` 改成 `word2[j-1]`，问题剩 `dp[i-1][j-1]`
   - 删除：删 `word1[i-1]`，问题剩 `dp[i-1][j]`
   - 插入：`word1` 末尾插 `word2[j-1]`，问题剩 `dp[i][j-1]`
2. 操作只有两种（插+删，不能替换）怎么办？→ LC 583，转化为求 LCS。
3. 求 **具体的操作序列**？→ 反向回溯 dp 表，每次根据来源决定是替/删/插。
4. 含 **不同操作代价**？→ 把 `+1` 改成各自代价 `+c_op`。
