...
2. **"求乘积最大子数组呢？"** → 负数 × 负数 = 正，要 **同时维护 max 与 min**，遇到负数交换。即 [LC 152](https://leetcode.cn/problems/maximum-product-subarray/)。
3. **"环形数组的最大子数组和呢？"** → 两种情况取最大：a) 不跨环→ Kadane；b) 跨环→ `total - 最小子数组和`。注意全负数要特判。即 [LC 918](https://leetcode.cn/problems/maximum-sum-circular-subarray/)。
4. **"允许删除一个元素后的最大子数组和？"** → 双状态 DP：`f[i][0]` 不删、`f[i][1]` 已删。即 [LC 1186](https://leetcode.cn/problems/maximum-subarray-sum-with-one-deletion/)。
...
### 同类型推荐（最大子段和家族）
- [LC 152. 乘积最大子数组](https://leetcode.cn/problems/maximum-product-subarray/)（同时维护 max/min）
- [LC 918. 环形子数组的最大和](https://leetcode.cn/problems/maximum-sum-circular-subarray/)（断环：`max子段和` vs `total - min子段和`）
- [LC 1186. 删除一次得到子数组最大和](https://leetcode.cn/problems/maximum-subarray-sum-with-one-deletion/)（双状态 DP）
- [LC 363. 矩形区域不超过 K 的最大数值和](https://leetcode.cn/problems/max-sum-of-rectangle-no-larger-than-k/)（前缀和 + 有序集合）
- [LC 209. 长度最小的子数组](../02-sliding-window/leetcode-209-minimum-size-subarray-sum.md)（同思路但目标改为和 ≥ s）
